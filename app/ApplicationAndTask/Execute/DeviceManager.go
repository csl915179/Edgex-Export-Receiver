...
func popTask(index int, deviceid string){
...
func failTask(index int, deviceid string) {
	deviceManagerVars.m.deviceMap[deviceid].taskList[index].Result = result{ExecResult:domain.Fail}
	deviceManagerVars.m.deviceMap[deviceid].taskList[index].State = executed
	popTask(index, deviceid)
}
...
func (m *devicemanager) ReloadDevices() {
	...
	for i:=0; i<len(deviceManagerVars.m.deviceMap[deviceList[i].Id.Hex()].taskList); i++ {
		failTask(i,deviceList[i].Id.Hex())
	}
	deviceManagerVars.mutex.Unlock()
}
...
func (m *devicemanager) RemoveDevice(id string) {
	deviceManagerVars.mutex.Lock()
	for i:=0; i<len(deviceManagerVars.m.deviceMap[id].taskList); i++ {
		failTask(i,id)
	}
...
func (m *devicemanager) ExecuteTasks(deviceid string, tasks []*TaskExecuteUnit) {
	...
	deviceManagerVars.m.deviceMap[deviceid] = deviceManage{lock:new(sync.Mutex),taskList:newTaskList}
...
func (dm deviceManage) Less(i, j int) bool{ return dm.taskList[i].Task.Command.TimeLimit < dm.taskList[j].Task.Command.TimeLimit}
func (m *devicemanager) allocateTasksForDevice(deviceid string){
	//定义如何执行任务,这里如果条件允许那么只有分配的CPU系数会不同
	execTask := func(task *TaskExecuteUnit, device *domain.Device, cpu_count int64, cpu_unit int64, index int) {
		...
		deviceManagerVars.m.deviceMap[deviceid].taskList[index].State = executing
		popTask(index, deviceid)
		go func() {
			time.Sleep(time.Duration(task.Task.Command.CPURequest/task.Cpu)*time.Second)
			deviceManagerVars.m.deviceMap[deviceid].taskList[index].Result = result{ExecResult:domain.OK}
			deviceManagerVars.m.deviceMap[deviceid].taskList[index].State = executed
			device.CpuUsed -= task.Cpu
			device.MemoryUsed -= task.Memory
			device.DiskUsed -= task.Disk
		...
		if avail_disk>=command.DiskRequest && avail_memory>=command.MemoryRequest && avail_netrate>=command.Size{
			...
			for ;cpu_count*cpu_unit<=avail_cpu; cpu_count++{
				if task.Task.Command.TimeLimit == 0 {
					execTask(task, &device, cpu_count, cpu_unit, i)
					break
				}
